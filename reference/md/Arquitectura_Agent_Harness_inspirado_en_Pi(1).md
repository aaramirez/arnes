...
# 32. Extensión v0.5 — De Agent Harness a Enterprise Agent Runtime

La arquitectura base inspirada en Pi permanece como núcleo. La versión empresarial agrega capas alrededor del core, no lógica condicional dentro del AgentLoop.

```text
External World
    ↓
ActivationGateway → AdmissionController → ActivationRouter
    ↓
WorkflowRuntime / AgentRuntime / JobRuntime
    ↓
AgentCommunicationGateway ↔ Internal Delegation / A2A Adapter
    ↓
ToolRuntime → PolicyEngine → CapabilityRegistry → CredentialBroker
    ↓
Enterprise Systems
```

Cross-cutting: durable state, tenant isolation, data governance, idempotency, execution ledger, immutable audit, observability, evals, FinOps and kill switches.

## 32.1 Activation contracts
`ActivationRequest` normalizes USER_REQUEST, API_REQUEST, WEBHOOK, QUEUE_MESSAGE, DOMAIN_EVENT, SCHEDULE, FILE_EVENT, DATABASE_EVENT, AGENT_REQUEST and SYSTEM_SIGNAL. `AdmissionController` returns ACCEPT, REJECT, DEFER, DEDUPLICATE, RATE_LIMIT or ROUTE.

## 32.2 Agent communication
The core contract is `AgentCommunicationGateway`; it supports discovery, delegation, messaging, cancellation and result delivery. Internal delegation and external federation are distinct. A2A is implemented as an adapter for independent agent systems.

## 32.3 Communication stack
```text
Semantic: Task · Message · Result · Artifact · Progress · Failure · Cancel
Protocol: Internal · A2A · Custom
Transport: In-process · HTTP · Webhook · WebSocket · SSE · gRPC · Queue · Kafka/Event Bus
Reliability: Ack · Retry · Timeout · Dedup · Idempotency · Ordering · DLQ · Backpressure
Security: Identity · AuthN/Z · Delegation · Encryption · Tenant Isolation
Governance: Policy · Audit · Budget · Rate Limits · Data Classification
```

## 32.4 Enterprise first-class components
ActivationGateway, AdmissionController, ActivationRouter, WorkflowRuntime, ExecutionFabric, AgentRegistry, AgentCommunicationGateway, DelegationManager, ExecutionGraph, CapabilityLifecycleManager, CredentialBroker, EnterpriseContextLayer, DataGovernance, RunStore, CheckpointStore, ArtifactStore, IdempotencyManager, ConcurrencyManager, ExecutionLedger, AuditLedger, EvaluationHarness, CertificationService, OutcomeTracker, ConfigurationRegistry, FeatureFlagService, SLOManager and operational kill switches.
