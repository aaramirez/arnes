...
# 25. Enterprise-runtime alignment amendment

The Book Harness is a Domain Pack over the Generic Harness. Its triggers MAY include user requests, repository commits, scheduled builds, review queues and other agents. All triggers MUST normalize through the Generic Activation Layer.

Book agents MUST communicate through `AgentCommunicationGateway`; internal editorial sub-agents may use internal delegation, while independently deployed reviewers may use an A2A adapter. Transport selection (HTTP, webhook, WebSocket, queue, event bus) remains outside editorial semantics.

Add to the self-hosting roadmap: Activation integration, durable editorial runs, ExecutionGraph for reviewer delegations, structured HandoffPackage for human editorial decisions, Evaluation Harness for chapter regression, and business/production outcome tracking for build quality and cost.
