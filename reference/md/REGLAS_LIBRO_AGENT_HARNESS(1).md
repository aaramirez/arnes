...
# 31. Amendment v0.5 — Reglas para arquitectura empresarial y comunicación

Cada capítulo que introduzca una frontera empresarial DEBE declarar explícitamente: semantic contract, protocol adapter, transport adapter, reliability semantics, security/policy boundary, state impact, failure semantics and observability.

## Regla de activación
Nunca escribir `Webhook -> AgentLoop`, `Queue -> AgentLoop` o `User -> AgentLoop` en la arquitectura empresarial. La forma canónica es `Trigger -> Adapter -> ActivationRequest -> Admission -> Route -> ExecutionTarget`.

## Regla de comunicación entre agentes
Nunca acoplar un agente a la implementación de otro. Usar `AgentCommunicationGateway`. Diferenciar: internal delegation, external federation, protocol y transport.

## Regla A2A
A2A se enseña como estándar de interoperabilidad en el boundary, no como dependencia del core. Todo pseudocódigo A2A debe aparecer detrás de un adapter.

## Regla de transporte
HTTP, webhook, WebSocket, SSE, gRPC, queues, Kafka, Service Bus y Pub/Sub son transportes/mecanismos de delivery. No deben definir la semántica del agente.

## Regla de enterprise readiness
Todo componente enterprise debe declarar tenant behavior, identity/authority, data classification, durability, idempotency, auditability, versioning and operational control cuando aplique.

## Regla de planos
Los diagramas de plataforma deben clasificar componentes en: Ingress, Execution, Agent Interoperability, Capability, Data, Control, Reliability, Observability/Governance y Execution Fabric.
